...
data {
  ...
  vector[n_year] s_eagle_sd;      // Eagle Sonar Passage sd 
  vector[n_year] r_mr;            // Radio Telemetry mark-recapture estimate
  vector[n_year] r_mr_sd;         // Radio Telemetry mark-recapture sd 
  ...
  vector[n_year] t_rtmr_sd;       // Radio Telemetry Mark Recapture Total
  ...
  vector[n_year] s_pilot_sd;      // Pilot Station Sonar sd
  vector[n_year] splt_exp_cv;     // CV during experimental period
  ...
  matrix[5, n_year] h_stock_l;
  matrix[6, n_year] h_stock_m;
  matrix[6, n_year] h_stock_c;
  ...
  int nye[2];
  int ytan;                      // This is no longer a year, but a year reference
  vector[16] usesclike;
  vector[11] canesclike;
  vector[3] canpaslike;
  vector[9] uspaslike;
  int n_ch;
  int n_sa;
  ...
  matrix [nyrs, A] H_comps_bp;  // Below pilot CDN harvest observed age composition in proportion by age class
  matrix [nyrs, A] H_comps_ap;  // Above pilot CDN harvest observed age composition in proportion by age class
  matrix [nyrs, A] H_comps_cdn; // CDN harvest observed age composition in proportion by age class
  matrix [nyrs, A] S_comps;     // Border observed age composition in proportion by age class
  vector [nyrs] ess_age_comp;   // Effective input sample size for age comp "observations"
...
parameters {
  ...
  vector<lower=0.0, upper=6.0>[n_ch] log_tchen;  // log transformed slope for Chena tower
  vector<lower=0.0, upper=6.0>[n_sa] log_tsalc;  // log transformed slope for Salcha tower
  vector<lower=0.0, upper=6.0>[n_hw] log_whenw;  // log transformed slope for Henshaw Weir
...
transformed parameters {
  ...
  // US escapement parameters slope
  vector[n_ch] tchen;       //  Slope for Chena tower
  vector[n_sa] tsalc;       //  Slope for Salcha tower
  vector[n_hw] whenw;       //  Slope for Henshaw Weir
  ...
  matrix[3, n_year] h_up;   // Above Pilot harvest by stock
  matrix[3, n_year] h_low;  // Below Pilot harvest by stock
  ...
  matrix[3, n_year] ph_up;  // Ob stock proportion above Pilot
  matrix[3, n_year] ph_low; // Ob stock proportion below Pilot
  ...
  matrix<lower=0>[nyrs, A] o_run;        // Observed run size by age class
  matrix<lower=0, upper=1>[nyrs, A] o_run_comp; // Observed age composition by year
...
model {
  ...
  // Pilot Sonar Passage
    ...
    if (s_pilot[i] >0) {
      // Fit to experimental years
	    if (i < nyp[1]) {
        log(s_pilot[i]) ~ normal(log(qplt[1]*plt_t[i]), sqrt(log(1+square(splt_exp_cv[i]))+square(rspilt[1]))); 
	    }else {
      // Pilot Staion in production
        if (i < nyp[2]) {
          log(s_pilot[i]) ~ normal(log(qplt[2]*plt_t[i]), sqrt(log(square(s_pilot_sd[i]/s_pilot[i])+1)+square(rspilt[2]))); 
        }else {
      // Pilot Station with DIDSON
          log(s_pilot[i]) ~ normal(log(plt_t[i]), sqrt(log(square(s_pilot_sd[i]/s_pilot[i])+1))); 
        } 
	    }
    }
    ...
    if (t_rtmr[i] >0) {
      // Fit to 2000-2001
      if(i < nyp[2]){
      log(t_rtmr[i]) ~ normal(log(plt_t[i]) , sqrt(log(square(t_rtmr_sd[i]/t_rtmr[i])+1)+square(rspilt[3])));
      }else {
        log(t_rtmr[i]) ~ normal(log(plt_t[i]) , sqrt(log(square(t_rtmr_sd[i]/t_rtmr[i])+1)+square(rspilt[4])));
      }
    }
    ...
    if (t_chen[i] >0) {
	    if (i < ytan) {
		    log(t_chen[i]) ~ normal(log(midesc[i]/tchen[1]), sqrt(log(square(t_chen_cv[i])+1)+square(rtower)));
	    }	else {
		    log(t_chen[i]) ~ normal(log(midesc[i]/tchen[n_ch]), sqrt(log(square(t_chen_cv[i])+1)+square(rtower)));
	    }
    }
    //Salcha Tower
    if  (t_salc[i] >0) {
	    if (i < ytan) {
		    log(t_salc[i]) ~ normal(log(midesc[i]/tsalc[1]), sqrt(log(square(t_salc_cv[i])+1)+square(rtower)));
	    }	else {
		    log(t_salc[i]) ~ normal(log(midesc[i]/tsalc[n_sa]), sqrt(log(square(t_salc_cv[i])+1)+square(rtower)));
	    }
    }
    //Henshaw Creek Weir
    if (w_henw[i] >0) {
      if (i < ytan) {
        log(w_henw[i]) ~ normal(log(midesc[i]/whenw[1]), sqrt(log(square(w_henw_cv[i])+1)+square(rtower)));
      } else {
	      log(w_henw[i]) ~ normal(log(midesc[i]/whenw[n_hw]), sqrt(log(square(w_henw_cv[i])+1)+square(rtower)));
	    }
    }
    //Chena Aerial
    if (a_chen[i] >0) {
	    if (i < ytan) {
         log(a_chen[i]) ~ normal(log(midesc[i]/achen/tchen[1]), sqrt(log(square(a_chen_cv[i])+1)+square(raerial)));
	    } else {
	      log(a_chen[i]) ~ normal(log(midesc[i]/achen/tchen[n_ch]) ,sqrt(log(square(a_chen_cv[i])+1)+square(raerial)));
	    }
    }
    //Salcha Aerial
    if (a_salc[i] >0) {
		  if (i < ytan) {
         log(a_salc[i]) ~ normal(log(midesc[i]/asalc/tsalc[1]), sqrt(log(square(a_salc_cv[i])+1)+square(raerial)));
		  } else {
	      log(a_salc[i]) ~ normal(log(midesc[i]/asalc/tsalc[n_sa]), sqrt(log(square(a_salc_cv[i])+1)+square(raerial)));
		  }
	  }
    ...
    // Eagle Sonar 
    if (s_eagle[i] >0) {
      log(s_eagle[i]) ~ normal(log(canpass[i]), sqrt(log(square(s_eagle_sd[i]/s_eagle[i])+1)));
    }
    // Radio Telemetry Mark Recapture
    if (r_mr[i] >0) {
      log(r_mr[i]) ~ normal(log(canpass[i]) , sqrt(log(square(r_mr_sd[i]/r_mr[i])+1)));
    }
  ...
  for(i in 1:n_year) {
    ...
    target += normal_lpdf(log(sum(h_up[,i])) | log(sum(eh_up[,i])), sqrt(log(square(hp_cv[i])+1)));  
    // Total harvests Lower river - SSQ
    target += normal_lpdf(log(sum(h_low[,i])) | log(sum(eh_low[,i])), sqrt(log(square(hp_cv[i])+1)));  
